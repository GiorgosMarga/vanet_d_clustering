...
fname=results_data_temperatures_cars_40_400m
...
python3 ./python_utils/plotAverageAccuracies.py ${fname}  "anp"

echo "Processing all rnp folders..."
python3 ./python_utils/plotAverageAccuracies.py ${fname} "rnp"

echo "Processing all snp folders..."
python3 ./python_utils/plotAverageAccuracies.py ${fname} "snp"

# === Process all anp combinations ===
echo "Plotting for ANP"
for gap in "${gaps[@]}"; do
    echo "Plotting for $gap"
    python3 ./python_utils/plotAverageAccuracies.py ${fname} "anp" "$gap"
done
for lap in "${laps[@]}"; do
    echo "Plotting for $lap"
    python3 ./python_utils/plotAverageAccuracies.py ${fname} "anp" "$lap"
done
...
for conc in "${rnp_concs[@]}"; do
    for gap in "${gaps[@]}"; do
        ...
        python3 ./python_utils/plotAverageAccuracies.py ${fname} "rnp_$conc" "$gap"
    done
    for lap in "${laps[@]}"; do
        echo "Plotting for RNP_$conc $lap"
        python3 ./python_utils/plotAverageAccuracies.py ${fname} "rnp_$conc" "$lap"
    done
done
